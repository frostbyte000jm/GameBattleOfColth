...
namespace BattleOfColthv2
{
	...
	{
		//declarations
		private Vector3 position, velocity, playerPos;
		private float shipHeight, shipWidth, speed, rotation;
		private List<Enemy> enemyList;
		private PlayerShip playership;
		
		public FighterShip (GraphicsContext g, Texture2D tex, PlayerShip pShip, List<Enemy> eList, int hp, int st, int x, int y):base(g, tex, hp, st)
		{
		...
		public override void Move ()
		{
			...
			//stop the motion of the ship. This is going to be for later when the Ship can shoot.
			if(Vector3.DistanceSquared (Pos, playerPos) < 5000)
			{
				velocity *= 0;
			}
